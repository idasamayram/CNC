...
def visualize_with_label_attribution_2(
        signal,
        attributions,
        label,
        method_name,
    ...
    axes_labels = ["X", "Y", "Z"]
    fig, axs = plt.subplots(3, 2, figsize=(10, 6))  # 3 rows, 2 columns
    ...
    for i in range(3):  # Loop over axes: X, Y, Z
        ...
        '''
        # use this for visualizing without percentile
        ...
        # axs[i, 0].set_title(f"Time Domain Relevance Heatmap over {axes_labels[i]}-Axis \n Average Attribution in this Axis: {avg_attributions[i]:.3f}", fontsize=8)
        axs[i, 0].set_title(f"Time Domain Relevance Heatmap over {axes_labels[i]}-Axis", fontsize=8)

        axs[i, 0].set_xlabel("Time Steps", fontsize=6)
        axs[i, 0].set_ylabel("Signal Value", fontsize=6)
        axs[i, 0].legend(fontsize=6, prop={'size': 6}, handlelength=1, handletextpad=0.5, borderpad=0.3, frameon=True,
                         framealpha=0.8)
        ...
        # use this for plotting exact attribution values
        # axs[i, 1].bar(time_steps, attributions[i], color=["red" if val > 0 else "blue" for val in attributions[i]], alpha=0.9, width=1.0)

        # use this for replacing positive and negative values with zero respectively to get the full relevance over time
        axs[i, 1].fill_between(range(len(attributions[i])), attributions[i], where=attributions[i] > 0, color='red', alpha=0.5, label='Positive Relevance')
        axs[i, 1].fill_between(range(len(attributions[i])), attributions[i], where=attributions[i] < 0, color='blue', alpha=0.5, label='Negative Relevance')

        axs[i, 1].set_title(f"Time Domain Relevance {axes_labels[i]}", fontsize=8)
        axs[i, 1].set_xlabel("Time Steps", fontsize=6)
        axs[i, 1].set_ylabel("Relevance Value", fontsize=6)
        axs[i, 1].legend(fontsize=6, prop={'size': 6}, handlelength=1, handletextpad=0.5, borderpad=0.3, frameon=True,
                         framealpha=0.8)
    ...
    fig.suptitle(f" Explanation for Time Domain Relevance\n with {method_name} - {label_text}", fontsize=12)  # Add overall title with label
...
# ------------------------
# this version uses separate title for each row, below version uses mutual title for each column
def visualize_lrp_single_sample_2(
        signal,
        relevance,
        label,
        sample_idx=0,
    ...
    """
    n_axes = signal.shape[0]
    fig, axs = plt.subplots(n_axes, 2, figsize=(10, 2 * n_axes))

    for i in range(n_axes):
        # Plot signal
        axs[i, 0].plot(signal[i], label=f"Signal ({axes_names[i]})", color='black', linewidth=0.8)
        axs[i, 0].set_title(f"Signal - Axis {axes_names[i]} (Label: {'Good' if label == 0 else 'Bad'})", fontsize=8)
        axs[i, 0].set_xlabel("Time Step", fontsize=6)
        ...
        axs[i, 1].set_title(f"LRP Relevance - Axis {axes_names[i]}" , fontsize=8)
        axs[i, 1].set_xlabel("Time Step", fontsize=6)
        axs[i, 1].set_ylabel("Relevance", fontsize=6)
        axs[i, 0].grid(visible=False)
        axs[i, 1].legend(fontsize=6, prop={'size': 6}, handlelength=1, handletextpad=0.5, borderpad=0.3, frameon=True,
                         framealpha=0.8)
        axs[i, 1].tick_params(axis='both', which='major', labelsize=6)
...
def visualize_lrp_single_sample(
    ...
    n_axes = signal.shape[0]
    ncols = 2  # 2 columns: signal, relevance
    nrows = n_axes
    figsize = (ncols * 5, nrows * 2)  # Add extra space for column titles
    fig = plt.figure(figsize=figsize)
    ...
    for i in range(n_axes):
        # Plot signal
        ax[i, 0].plot(signal[i], label=f"Signal ({axes_names[i]})", color='black', linewidth=0.8)
        if i == n_axes - 1:
            ax[i, 0].set_xlabel("Time Step", fontsize=8)
        ...
        # Plot relevance (positive in red, negative in blue)
        ax[i, 1].fill_between(range(len(relevance[i])), relevance[i], where=relevance[i] > 0, color='red', alpha=0.5,
                              label='Positive Relevance')
        ax[i, 1].fill_between(range(len(relevance[i])), relevance[i], where=relevance[i] < 0, color='blue', alpha=0.5,
                              label='Negative Relevance')
        if i == n_axes - 1:
            ax[i, 1].set_xlabel("Time Step", fontsize=8)
        ax[i, 1].set_ylabel(f"{axes_names[i]}\nRelevance", fontsize=8)
        ax[i, 1].grid(visible=False)
        ax[i, 1].legend(fontsize=6, loc="upper right", prop={'size': 6}, handlelength=1, handletextpad=0.5,
                        borderpad=0.3, frameon=True, framealpha=0.8)
    ...
    fig.suptitle(f"LRP Explanation - ({label_text})", fontsize=14)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.show()
...
# this version only lacks the mutual title and labels
def visualize_lrp_dft_2(
        relevance_time,
        relevance_freq,
        signal_freq,
        input_signal,
    ...
    for i in range(n_axes):
        ...
        # Plot signal on top
        ax[i, 0].plot(x_time, signal_time_axis, color="black", linewidth=0.8, label="Signal")
        ax[i, 0].set_xlabel("Time (s)", fontsize=10)
        ax[i, 0].set_ylabel("Amplitude", fontsize=10)
        # ax[i, 0].set_title(f"Time Domain Relevance Heatmao over Axis {axes_names[i]}\n{label_text}, Avg Relevance: {avg_relevances_time[i]:.4f}", fontsize=14)
        ax[i, 0].set_title(f"Time Domain Relevance Heatmap \n over Signal- Axis {axes_names[i]}- {label_text}", fontsize=12)
        ...
        ax[i, 1].set_xlabel("Time (s)", fontsize=12)
        ax[i, 1].set_ylabel("Relevance", fontsize=12)
        ax[i, 1].set_title(f" Relevance in Time Domain - Axis {axes_names[i]}", fontsize=12)
        ax[i, 1].grid(visible=False)
        ax[i, 1].legend(fontsize=6, loc="upper right", prop={'size': 10}, handlelength=1, handletextpad=0.5, borderpad=0.3, frameon=True, framealpha=0.8)
        ax[i, 1].tick_params(axis='both', which='major', labelsize=8)
        ...
        ax[i, 2].set_xlabel("Frequency (Hz)", fontsize=12)
        ax[i, 2].set_ylabel("Magnitude", fontsize=12)
        # ax[i, 2].set_title(f"Frequency Domain Relevance Heatmap over Axis {axes_names[i]}\nAvg Relevance: {avg_relevances_freq[i]:.4f}",fontsize=14)
        ax[i, 2].set_title(f"Frequency Domain Relevance Heatmap \n over Signal- Axis {axes_names[i]}",fontsize=12)
        ...
        ax[i, 3].set_xlabel("Frequency (Hz)", fontsize=12)
        ax[i, 3].set_ylabel("Relevance", fontsize=12)
        ax[i, 3].set_title(f"Relevance in Frequency Domain- Axis {axes_names[i]}", fontsize=12)
    ...
    plt.tight_layout()
    plt.show()
# a bit cleaned up version, best so far for time and freq domain WITH MUTUAL TITLE FOR COLUMNS
def visualize_lrp_dft(
    ...
    fig.suptitle(f"LRP Explanation - {label_text}", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.show()
...
# this version differs from the below one in titles and labels onlly
def visualize_hybrid_timefreq_2(
        relevance_time,
        relevance_freq,
        signal_freq,
        relevance_timefreq,
    ...
    for i in range(n_axes):
        ...
        try:
            if relevance_timefreq is not None and not np.all(relevance_timefreq[i] == 0):
                ...
                ax[i, 5].set_xlabel("Time (s)", fontsize=12)
                ax[i, 5].set_ylabel("Frequency (Hz)", fontsize=12)
                ax[i, 5].set_title(f"Relevance in Time-Frequency Domain - Axis {axes_names[i]}", fontsize=12)
                ax[i, 5].grid(visible=False)
                ax[i, 5].legend(fontsize=6, loc="upper right", prop={'size': 10}, handlelength=1, handletextpad=0.5,
                                borderpad=0.3, frameon=True, framealpha=0.8)
                ax[i, 2].tick_params(axis='both', which='major', labelsize=8)
                plt.colorbar(im2, ax=ax[i, 5], label="Relevance")
            else:
                ...
                ax[i, 5].set_xlabel("Time (s)", fontsize=12)
                ax[i, 5].set_ylabel("Frequency (Hz)", fontsize=12)
                ax[i, 5].set_title(f"Time-Frequency Relevance Spectrogram - Axis {axes_names[i]}", fontsize=12)
                ax[i, 5].set_ylim(0, k_max)
...
def visualize_hybrid_timefreq(
    ...
    fig.suptitle(f"DFT-LRP Explanation - {label_text}", fontsize=20)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.show()
