...
class VibrationDataset(Dataset):
    '''
    Returns both time and frequency domain data for each sample.
    '''
    ...
    def __getitem__(self, idx):
        ...
        data = np.transpose(data, (1, 0))  # (3, 2000)
        freq_data = np.abs(np.fft.rfft(data, axis=1))  # (3, freq_len)
...
class FrequencyDomainCNN(nn.Module):
    ...
    def forward(self, x):
        if isinstance(x, (tuple, list)):
            # Handle case with separate time and frequency inputs
            ...
            # Input shape check and correction
            if len(x.shape) == 4:  # [batch, channels, 3, time_steps]
                # Reshape to [batch, 3, time_steps]
                batch_size = x.shape[0]
                time_steps = x.shape[3]
                x = x.reshape(batch_size, 3, time_steps)
            ...
            # Process the frequency representation
...
if __name__ == "__main__":
    ...
    # Example: get a batch and select a single sample
    sample_batch, label_batch = next(iter(test_loader))
    random_index = np.random.randint(0, sample_batch[0].shape[0])
    single_time = sample_batch[0][random_index].unsqueeze(0).to(device)
    single_freq = sample_batch[1][random_index].unsqueeze(0).to(device)
    single_label = label_batch[random_index]
    with torch.no_grad():
        output = freq_model((single_time, single_freq))
        pred = torch.argmax(output, dim=1).item()
    print(f"Random Index: {random_index}, Predicted_label: {pred}, True_label: {single_label.item()}")
